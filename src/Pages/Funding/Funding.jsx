...
const Funding = () => {
  ...
  const fetchFunds = async () => {
    ...
    try {
      ...
      setFunds(data); // replace funds completely
    } catch (err) {
      console.error(err);
      toast.error("Failed to fetch funding data", { duration: 3000 });
    } finally {
      setLoading(false);
    }
  };
    ...
    const handleStripeSuccess = async () => {
      ...
      try {
        const res = await fetch(
          `${import.meta.env.VITE_API_URL}/api/funding/confirm-payment`,
          {
            method: "POST",
        ...
        // Always fetch funds once after Stripe confirmation
        fetchFunds();
      }
    };

    handleStripeSuccess();
    // Only call fetchFunds if there is no session_id
...
;
  ...
  const handleGiveFund = async () => {
    ...
    if (amt < 60) {
      toast.error("Amount must be at least 60 BDT", { duration: 3000 });
      return;
    }
    ...
    try {
      ...
      if (data.url) {
        ...
        toast.error("Failed to create Stripe session", { duration: 3000 });
      }
    } catch (err) {
      console.error(err);
      toast.error("Payment initiation failed", { duration: 3000 });
    }
  };
